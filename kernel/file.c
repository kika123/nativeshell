/* $Id: file.c 50983 2011-03-06 13:28:11Z pschweitzer $
 *
    ...
    )
{
    ...
    {
        HeapFree(RtlGetProcessHeap(), 0, replacedW);
        SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }
    ...
    if (lpBackupFileName)
    {
        ...
        {
            HeapFree(RtlGetProcessHeap(), 0, replacedW);
            HeapFree(RtlGetProcessHeap(), 0, replacementW);
            SetLastError(ERROR_INVALID_PARAMETER);
            return FALSE;
        }
    }
    ...
    HeapFree(RtlGetProcessHeap(), 0, replacedW);
    HeapFree(RtlGetProcessHeap(), 0, replacementW);
    HeapFree(RtlGetProcessHeap(), 0, backupW);
...
Cleanup:
    ...
    if (NtReplacementName.Buffer)
        RtlFreeHeap(RtlGetProcessHeap(), 0, NtReplacementName.Buffer);
    if (NtReplacedName.Buffer)
        RtlFreeHeap(RtlGetProcessHeap(), 0, NtReplacedName.Buffer);
