...
static BOOL add_boot_rename_entry( LPCWSTR source, LPCWSTR dest, DWORD flags )
{
    ...
    {
        if (!(Buffer = HeapAlloc(RtlGetProcessHeap(), 0, DataSize + len1 + len2 + sizeof(WCHAR))))
            goto Quit;
	...
	else
	{
 	        if (folder==FALSE)
		{
		 ...
		 else
		 {
		   ...
		   lpDeleteFile = (LPWSTR) HeapAlloc(RtlGetProcessHeap(),HEAP_ZERO_MEMORY,max_size * sizeof(WCHAR));
		   if (lpDeleteFile == NULL)
		       return FALSE;

		   lpNewFileName2 = (LPWSTR) HeapAlloc(RtlGetProcessHeap(),HEAP_ZERO_MEMORY,max_size * sizeof(WCHAR));
		   if (lpNewFileName2 == NULL)
		   {
		     HeapFree(RtlGetProcessHeap(),0,(VOID *)  lpDeleteFile);
			 return FALSE;
		   }

		   lpExistingFileName2 = (LPWSTR) HeapAlloc(RtlGetProcessHeap(),HEAP_ZERO_MEMORY,max_size * sizeof(WCHAR));
		   if (lpExistingFileName2 == NULL)
		   {
		     HeapFree(RtlGetProcessHeap(),0,(VOID *)  lpNewFileName2);
		     HeapFree(RtlGetProcessHeap(),0,(VOID *) lpDeleteFile);
		     return FALSE;
		   }
		       ...
		       if (!loop)
		       {
		         ...
		         if (wcsncmp(lpExistingFileName,lpExistingFileName2,size))
		         {
				   ...
				   if (size+6>max_size)
				   {
		              ...
		              if (lpExistingFileName2 != NULL)
		                  HeapFree(RtlGetProcessHeap(),0,(VOID *) lpExistingFileName2);
